...
def step2_extract_google_docs(html_content):
    """Step 2: Extract Google Doc links from 'name' column"""
    print("Step 2: Extracting Google Doc links...")
    
    # Simple regex to find Google Doc links
    doc_pattern = r'https://docs\.google\.com/document/d/[a-zA-Z0-9_-]+[^"\s<>]*'
    doc_links = re.findall(doc_pattern, html_content)
    
    # Remove duplicates
    doc_links = list(set(doc_links))
    
    print(f"✓ Found {len(doc_links)} Google Doc links")
    return doc_links
...
def main():
    ...
    # Step 2: Extract Google Doc links
    doc_links = step2_extract_google_docs(html_content)
    
    # Process each doc (for now, just the first one as example)
    for i, doc_url in enumerate(doc_links[:1]):  # Limit to 1 for testing
        print(f"\nProcessing doc {i+1}/{len(doc_links)}")
        
        # Step 3: Scrape doc content
        doc_content = step3_scrape_doc_contents(doc_url)
        
        if doc_content:
            # Step 4: Extract links
            links = step4_extract_links(doc_content)
            
            # Step 5: Download links (with placeholder data mapping)
            # TODO: Extract actual name/email/type from sheet context
            name = f"Person_{i+1}"
            email = f"person{i+1}@example.com"
            type_info = "Unknown"
            
            downloaded_items = step5_download_links(links, name, email, type_info)
