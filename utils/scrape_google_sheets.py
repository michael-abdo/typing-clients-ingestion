...
def download_sheet():
    ...
    logger.info(f"Downloading Google Sheet from {URL}...")
    ...
    logger.info(f"Google Sheet HTML saved to {CACHE_FILE}")
    return content

def get_sheet_html():
    """Get the sheet HTML, either from cache or by downloading"""
    if os.path.exists(CACHE_FILE):
        logger.info(f"Using cached Google Sheet HTML from {CACHE_FILE}")
...
def fetch_table_data():
    ...
    logger.info(f"HTML retrieved, looking for specific div ID {target_div_id}...")
    
    # Look for the specific div with configured ID
    target_div = soup.find("div", {"id": target_div_id})
    if target_div:
        logger.info(f"Found the target div with ID {target_div_id}")
        # Find table inside this div
        table = target_div.find("table")
    else:
        logger.warning(f"Div with ID {target_div_id} not found, falling back to generic table search")
    ...
    if table:
        ...
        logger.debug(f"Printing first 10 rows to diagnose structure:")
        for i in range(min(10, len(rows))):
            logger.debug(f"Row {i}, cells: {len(rows[i].find_all('td'))}")
        ...
        if len(rows) > 15:
            ...
            for j, cell in enumerate(cells):
                ...
                for k, link in enumerate(links):
                    ...
                    logger.debug(f"Link {k}: text='{link_text}', href='{href}'")
                    if href.startswith("https://www.google.com/url?q="):
                        actual_url = extract_actual_url(href)
                        logger.debug(f"  Extracted URL: {actual_url}")
            
        # Let's look at the cells in row 15 which previously had links
        if len(rows) > 15:
            logger.debug(f"Examining row 15 which previously had links:")
            row15_cells = rows[15].find_all("td")
            logger.debug(f"Number of cells: {len(row15_cells)}")
            if len(row15_cells) > 2:
                link_cell = row15_cells[2]
                logger.debug(f"Link cell text: {link_cell.get_text(strip=True)}")
                a_tags = link_cell.find_all("a")
                logger.debug(f"Number of <a> tags: {len(a_tags)}")
                for a_tag in a_tags:
                    logger.debug(f"<a> tag href: {a_tag.get('href', 'None')}")
        ...
        logger.debug("Trying different row indices to find links...")
        for start_row in [15, 16, 17, 18, 19, 20]:
            logger.debug(f"Trying from row index {start_row}...")
            temp_records = []
                ...
                if len(cells) >= 3:
                    ...
                    if a_tag and a_tag.has_attr("href"):
                        ...
                        if link and (link.startswith("http") or link.startswith("/")):
                            ...
                            if chapter and doc_name:
                                ...
                                logger.debug(f"Found valid link in row {row_index}: {link}")
                                temp_records.append({
                                    "name": entry_name,
                                    "link": link,
                                    "type": chapter
                    ...
                    # If no link tag but text looks like a URL
                        ...
                        if chapter and doc_name:
                            ...
                            logger.debug(f"Found URL text in row {row_index}: {link_text}")
                            temp_records.append({
                                "name": entry_name,
                                "link": link_text,
                                "type": chapter
            ...
            if temp_records:
                logger.info(f"Found {len(temp_records)} records from start row {start_row}")
                records.extend(temp_records)
                break
                
        # If we still don't have records, try extracting from the Google URLs directly
        if not records:
            logger.debug("Looking for any Google redirect URLs in all rows...")
            ...
            if header_row is None or name_index is None or type_index is None:
                logger.warning("Could not find header row with 'Name' and 'Type' columns")
                # Fall back to default indexes
                name_index = 2  # Try 2 instead of 3
                type_index = 5  # Try 5 instead of 6
            
            logger.debug(f"Using columns: Name={name_index}, Type={type_index}")
